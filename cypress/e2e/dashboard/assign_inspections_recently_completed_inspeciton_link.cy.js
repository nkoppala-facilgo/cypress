describe("Session Login ", () => {
    ...
    describe("Assign Inspections", function () {
        it("Assign Inspections : Recently Completed-To Verify user can able to click the Inspeciton link <smoke>", function () {
            cy.visit();
            cy.wait(7000);
            ...
            cy.wait(5000);
            cy.get('.assign-inspection').find('table').find('tbody').find('tr').first().find('td').eq(3).find('a').click({force: true});
            cy.wait(3000);
            cy.get('.assign-inspection').find('table').find('tbody').find('tr').first().find('td').eq(6).find('a').click({force: true});
            cy.wait(3000);
            cy.get('.assign-inspection').find('table').find('tbody').find('tr').first().find('td').eq(7).find('a').click({force: true});
        });
    });
});
