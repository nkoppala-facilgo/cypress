describe('Session Login ',()=>{
        beforeEach(()=>{
          ...
          describe("Count Credit Memos", function () {
                it("Count Credit Memos <smoke>", function() {
                        cy.visit()
                        cy.wait(3000)
                        cy.get('div[class="btn-document-content"]').contains("Credit Memos")
                        .parent().find('small')
                        .invoke('text')
                        .then(text => {
                                cy.get('div[class="btn-document-content"]').contains("Credit Memos").dblclick({force:true})
                                cy.wait(25000)
                                if(Number(text) < 20){
