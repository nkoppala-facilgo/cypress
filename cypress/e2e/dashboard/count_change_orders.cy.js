describe("Session Login ", () => {
        ...
        describe("Count Change orders", function () {
                it("Count Change orders <smoke>", function() {
                        cy.visit()
                        cy.wait(3000)
                        cy.get('div[class="btn-document-content"]').contains("Change Orders")
                        .parent().find('small')
                        .invoke('text')
                        .then(text => {
                                cy.get('div[class="btn-document-content"]')
                                .contains("Change Orders").dblclick({force:true})
                                cy.wait(25000)
                                if(Number(text) < 20){
