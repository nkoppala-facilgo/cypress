describe("Session Login ", () => {
    ...
    describe("Documents -> Project/Matrices -> Matrices", function () {
        it("fc-3856 Action Delete Matrix Button : Action-Delete the Matrix.", function () {
            ...
            .then(text => {
                cy.execute('script/Matrices/action_delete_matrix',this.data);
                cy.get(".matrices-view").find("tbody").find("tr").last().find("td").eq(1).invoke('text')
                .then(text1 => {
                    expect(Number(text)-1).to.equal(Number(text1));
                })
            })
        });
    });
