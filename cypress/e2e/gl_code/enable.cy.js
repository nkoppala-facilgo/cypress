describe('Session Login ',()=>{
   beforeEach(() => {
        ...
        cy.fixture(`data/${data_path}/gl_code/enable/data`).then(function (data) {
           this.data = data;
        })
     })
    describe("To verify Enable button functionality.", function () {
      it('FC-2215 Create and Enable  gl code <smoke>', function () { 
           ...
           cy.execute('/script/gl_code/disable',this.data)
           cy.wait(5000)
           cy.contains(ans).parent().parent().find('.text-center').find('.dropdown').find('.dropdown-toggle').click()
           cy.execute('/script/gl_code/enable',this.data)
       });
     });
   });
