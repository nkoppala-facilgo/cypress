...
describe("Session Login ", () => {
    ...
    describe("To verify that Invoice is going for QC Process if Invoice Processing QC Process='Yes'.", function () {
      it("FC-1118 Upload Invoice image documents. <smoke>", function () {
        cy.visit();
      ...
      it("FC-3890 To verify that DE user is able to 'Approve' Invoice. <smoke>", function () {
        var data_path = Cypress.env("data");
        cy.fixture(`data/${data_path}/login/data`).then(function (data) {
        ...
        cy.get("li.list-group-item.btn-show-document-head")
          .first()
          .dblclick({ force: true });
        cy.contains("a.btn.btn-primary", "Create Invoice").click({ force: true });
        ...
        cy.wait(4000);
        cy.contains('QC process has been completed.').should('exist');
      });
    });
  });
