...
describe("Session Login ", () => {
    beforeEach(() => {
      ...
      cy.fixture(`data/${data_path}/invoice_processing/resend_rejected_invoice_for_qc_process/data`).then(
        function (data) {
          this.data = data;
        }
      );
    });
    describe("To verify that DE user is able to edit rejected Invoice and resend back for the QC Process.", function () {
      it("FC_3540 Upload Invoice image documents. <smoke>", function () {
        cy.visit();
      ...
      it("FC-3891 To verify that DE user is able to 'Reject' Invoice and the rejected invoice is not editable <smoke>", function () {
        var data_path = Cypress.env("data");
        cy.fixture(`data/${data_path}/login/data`).then(function (data) {
        ...
        cy.select_by_label_new('Requester:',this.data['requester']);
        cy.select_by_label_new('Assignee:',this.data['assignee']);
        ...
        cy.contains('a','Invoice Images').click({force:true});
        cy.get('.fa-filter').click({multiple:true})
        cy.execute('/script/invoice_processing/search_rejected_invoice',this.data)
        cy.get('input[value=Search]').click();
        cy.get('li.list-group-item').its('length').should('be.gt', 0);
        cy.contains('em.status','Rejected').first().should('exist');
      });
    });
  });
