  ...
  beforeEach(() => {
    ...
    cy.fixture(`data/${data_path}/login/data`).then(function (data) {
      cy.login_with_session(
        ...
        data.technician.password
      );
    });
    cy.fixture(`data/${data_path}/work_order/approval_more_than_nte/data`).then(function (
      data
    ) {
      this.data = data;
    });
    cy.fixture(`data/${data_path}/work_order/less_nte_next_step/data`).then(function (data) {
      this.data1 = data;
    });
  });
  describe("To verify work order should not go for approval when order amount is less than NTE amount is set for user", function () {
    it("schedule a work order and create a non-catalog-order from it  <smoke>", function () {
