describe('Session Login ',()=>{
    beforeEach(() => {
        ...
        cy.fixture(`data/${data_path}/work_order/administrator_action_items_widget/declined_or_cancelled_emergency/data`).then(function (data) {
            this.data = data;
        })
    })

    describe("Administrator action items widget", function () {
		it("FC-11145 Review and Resend: To verify that Created Sdeclined Work Orders are displayed in the Admin Action Items widget's Declined or Cancelled. Review and Resend section emergency <smoke>", function () { 
			cy.visit();
        	cy.wait(4000);
            cy.get('#graph-content-0').find('tbody').find('tr').eq(7).find('td').eq(2).find('a').invoke('text')
            .then(text => {
				const count1 = text;
