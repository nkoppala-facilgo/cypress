describe('Session Login ',()=>{
    beforeEach(() => {
        ...
        cy.fixture(`data/${data_path}/login/data`).then(function (data) {
         ...
         cy.fixture(`data/${data_path}/work_order/administrator_action_items_widget/declined_or_cancelled_nextavailable/data`).then(function (data) {
            this.data = data;
        })
    })
    ...
    describe("Administrator action items widget", function () {
		it('FC-6755 and FC-3581 To verify Work Order status should not QuoteClosed when it has open Orders/Work Order - Quote Request - Quote - Order Flow  <smoke>', function () { 
			cy.visit();
        	cy.wait(4000);
            cy.get('#graph-content-0').find('tbody').find('tr').eq(7).find('td').eq(3).find('a').invoke('text')
            .then(text => {
				const count1 = text;
