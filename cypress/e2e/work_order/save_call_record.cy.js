describe('Session Login ',()=>{
    beforeEach(() => {
        ...
        cy.fixture(`data/${data_path}/work_order/save_call_record/data`).then(function (data) {
        this.data = data;
        })
        })
    describe("WO call record", function() {
        it('fc-6895 Verify Save Call Record button functionality <smoke>', function() {
            cy.visit()
            cy.on('uncaught:exception', (err, runnable) => { return false })
            ...
            cy.contains('button','Save Call Record').click()
            cy.wait(3000)
