describe("Fetch Work Orders", function () {
    beforeEach(() => {
        ...
        cy.fixture(`data/${data_path}/api/wo_with_all_extra_attributes/data`).then(function (data) {
            this.data1 = data;
        });
    });
    it('Create_Post_Api', function () {
       ...
       // submit work order
            cy.request({
              ...
              expect(result.body[0].company.name).to.eq("SFR FKH preview");
              expect(result.body[0].property.propertyName).to.eq("23504 Avacado Ave Port Charlotte, FL 33980");
              expect(result.body[0].unit.name).to.eq("23509702");
              expect(result.body[0].assignee.firstName).to.eq("SFR FKH");
              expect(result.body[0].address.address1).to.eq("23504 Avacado Ave");
              expect(result.body[0].workOrderPriority.priorityCode).to.eq("Next Available");
              expect(result.body[0].workOrderType.typeName).to.eq("Plumbing");
              expect(result.body[0].workOrderSubtype.typeName).to.eq("Plumbing- Other");
            });
        });
    });
});
