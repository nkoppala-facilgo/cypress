describe("Fetch Work Orders", function () {
    beforeEach(() => {
        ...
        cy.fixture(`data/${data_path}/api/wo_with_multiple_status_and_created_at_range/data`).then(function (data) {
            this.data1 = data;
        });
    });
    it('Create_Post_Api', function () {
       ...
       // submit work order
            cy.request({
               ...
               expect(result.body.workOrderItems.length).to.be.greaterThan(0);
                ...
                cy.request({
                    ...
                    body: {
                        ...
                        "statuses": this.data1['statuses'],
                        "created_date_from" : this.data1['created_date_from'],
                        "created_date_to" : this.data1['created_date_to'],
             ...
             expect(result.body[0].status).to.eq(this.data1['statuses']);
             expect(result.body[0].workOrderItems[0].itemName).to.eq("Plumbing, Plumbing- Other, Next Available");
             expect(result.body[0].workOrderItems[0].unit.name).to.eq("23509702");
            });
        });
    });
});
