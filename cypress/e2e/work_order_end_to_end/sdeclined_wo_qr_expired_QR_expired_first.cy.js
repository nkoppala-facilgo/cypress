        ...
        it("FC-11005 Verify WO Status is SDeclined -WO has an expired QR and a closed Quote, if the QR expired first", function () {
           ...
           this.data.quote_expires=quote_expires
            ...
            cy.contains("label", "WO#:").parent().find("input[type=text]").invoke("val").then((wo_number) => {
                var data_path = Cypress.env(`data`)
                cy.readFile(`cypress/fixtures/data/${data_path}/work_order_end_to_end/sdeclined_wo_qr_expired_QR_expired_first/data.json`).then((obj) => {
                    obj['num_no'] = wo_number;
                    cy.writeFile(`cypress/fixtures/data/${data_path}/work_order_end_to_end/sdeclined_wo_qr_expired_QR_expired_first/data.json`, obj);
                });
